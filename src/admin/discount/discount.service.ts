...
export class DiscountService {
  ...
  async create(data: CreateDiscountDto) {
    ...
    if (data.expireDay) {
        ...
        expirationDate = new Date(today.getTime() + data.expireDay * 24 * 60 * 60 * 1000);
    } else {
        expirationDate = new Date(data.expireDate);
    }

    const today = new Date();
    const expireDayLeft = Math.max(data.expireDay ? data.expireDay : 0, Math.ceil((expirationDate.getTime() - today.getTime()) / (1000 * 3600 * 24)));
...
async getAll() {
    ...
    const discountsWithTimeLeft = discounts.map(discount => {
        ...
        const expireDayLeft = Math.max(discount.expireDay, Math.ceil((expirationDate.getTime() - today.getTime()) / (1000 * 3600 * 24)));
        const expireTimeLeft = expirationDate.getTime() - today.getTime();
